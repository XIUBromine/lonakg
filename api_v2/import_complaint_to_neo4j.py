...
COMPLAINT_ROOT = os.getenv("COMPLAINT_ROOT", "")
...
ON CREATE SET c.created_at = row_ts, c.updated_at = row_ts, c.source = 'complaint'
ON MATCH SET
    c.updated_at = CASE
...
def import_complaint(root: str, database: str) -> Dict[str, int]:
                    ...
                    for _, row in chunk.iterrows():
                        ...
                        if len(pending_rows) >= BATCH_SIZE:
                            ...
                            if written_rows % PROGRESS_INTERVAL == 0:
                                print(f"[complaint] written={written_rows} total={total_rows} skipped={skipped_rows}")
                            pending_rows = []
...
def main() -> None:
    if COMPLAINT_ROOT == "":
        raise ValueError("COMPLAINT_ROOT is required")
    result = import_complaint(COMPLAINT_ROOT, DEFAULT_DB)
    print(
        "[complaint] done "
