...
FOREACH (_ IN CASE WHEN row.card_no_key IS NULL OR row.card_phone_num_key IS NULL THEN [] ELSE [1] END |
    MERGE (card_apply:card_no {key: row.card_no_key})
    MERGE (phone_card_apply:phone_num {key: row.card_phone_num_key})
    MERGE (card_apply)-[r4:order_apply_card_phone_num]->(phone_card_apply)
...
FOREACH (_ IN CASE WHEN row.repay_card_no_key IS NULL OR row.repay_card_phone_key IS NULL THEN [] ELSE [1] END |
    MERGE (card_repay:card_no {key: row.repay_card_no_key})
    MERGE (phone_repay:phone_num {key: row.repay_card_phone_key})
    MERGE (card_repay)-[r6:order_repay_card_phone_num]->(phone_repay)
